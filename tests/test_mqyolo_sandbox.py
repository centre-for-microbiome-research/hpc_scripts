...
def test_mqbedrock_setup_creates_missing_static_profile_stanza(tmp_path):
    # Existing users have [profile bedrock] only. Setup with a new name must
    # add that profile's region stanza so a later refresh's check() succeeds.
    home = tmp_path / "home"
    (home / ".aws").mkdir(parents=True)
    cfg = home / ".aws" / "config"
    ...
    p = _setup_claude(home, "--static-profile", "bedrock-alt")
    assert p.returncode == 0, p.stdout + p.stderr
    assert "added [bedrock-alt] region = ap-southeast-2" in p.stdout
    text = cfg.read_text()
    assert "[profile bedrock-alt]" in text
    assert text.index("[profile bedrock-alt]") < text.index(
        "region = ap-southeast-2", text.index("[profile bedrock-alt]")
    )
    settings = json.loads((home / ".claude" / "settings.json").read_text())
...
@pytest.mark.parametrize("setup_option", [
    "--setup-codex", "--setup-claude", "--setup-opencode",
])
def test_mqbedrock_setup_needs_no_aws_cli_or_cluster_pixi(tmp_path, setup_option):
    ...
    elif setup_option == "--setup-claude":
        assert (home / ".claude" / "settings.json").exists()
    else:
        assert (home / ".config" / "opencode" / "bedrock.json").exists()
